/* Host tests for PID-bound, serialized Toolbox injection coordination. */
...
using onion::ToolboxInjectionCoordinator;
...
int test_injects_and_publishes_current_pid() {
  ...
  ToolboxInjectionCoordinator coordinator;
  int inject_count = 0;
  constexpr pid_t kPid = 4100;

  const auto outcome = coordinator.ensure(
      []() -> pid_t { return kPid; },
      [&](pid_t pid) -> bool {
        ++inject_count;
  ...
  TEST_ASSERT_TRUE(coordinator.is_ready_for(kPid));
  reset_toolbox_marker();
  return 0;
}
...
int test_new_pid_replaces_stale_marker() {
  ...
  TEST_ASSERT_TRUE(outcome.result == ToolboxInjectionResult::Injected);
  TEST_ASSERT_EQ_INT(1, inject_count);
  TEST_ASSERT_TRUE(!coordinator.is_ready_for(kOldPid));
  TEST_ASSERT_TRUE(coordinator.is_ready_for(kNewPid));
  reset_toolbox_marker();
  return 0;
}
...
int test_failed_or_timed_out_injection_does_not_stick() {
  ...
  auto failed = coordinator.ensure(
  ...
  auto timed_out = coordinator.ensure(
